...
class MoveGroupClient(Node):
    ...
    def send_goal(self, target_pose: PoseStamped):
        ...
        goal_msg.request.group_name = 'manipulator'  # ← 修改为你的规划组名称
        goal_msg.request.pipeline_id = 'ompl'

        goal_msg.request.workspace_parameters.header.frame_id = target_pose.header.frame_id
        goal_msg.request.workspace_parameters.min_corner.x = -0.6
        goal_msg.request.workspace_parameters.min_corner.y = -0.6
        goal_msg.request.workspace_parameters.min_corner.z = 0.0
        goal_msg.request.workspace_parameters.max_corner.x = 0.6
        goal_msg.request.workspace_parameters.max_corner.y = 0.6
        goal_msg.request.workspace_parameters.max_corner.z = 1.2
...
def main(args=None):
    ...
    print("y ∈ [-0.6, 0.6] 米")
    print("z ∈ [0.6, 1.2] 米")
    print("输入 'disconnect' 可随时退出程序")

    while rclpy.ok():
        # 每次循环都重新获取当前末端位姿
        current_pose = node.get_current_end_effector_pose()
        if current_pose is None:
            node.get_logger().error("获取当前末端位姿失败，跳过此次输入")
            continue

        try:
            x_str = input("请输入目标位置 x (米): ").strip()
            if x_str.lower() == 'disconnect':
                print("已断开虚拟机器人连接，程序退出")
                break
            y_str = input("请输入目标位置 y (米): ").strip()
            if y_str.lower() == 'disconnect':
                print("已断开虚拟机器人连接，程序退出")
                break
            z_str = input("请输入目标位置 z (米): ").strip()
            if z_str.lower() == 'disconnect':
                print("已断开虚拟机器人连接，程序退出")
                break
            ...
            node.get_logger().error("输入格式错误，请输入数字")
            continue

        if not (-0.6 <= x <= 0.6 and -0.6 <= y <= 0.6 and 0.6 <= z <= 1.2):
            node.get_logger().error("输入坐标超出范围，请重新输入")
            continue
        ...
        target_pose.pose.orientation = current_pose.transform.rotation  # 使用当前实时末端朝向
